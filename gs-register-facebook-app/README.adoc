...
The main thing to note from the application settings page is the **App ID** and **App Secret** near the top. These values are your application's credentials to Facebook. You need these credentials to do almost anything with Facebook, including going through the OAuth authorization flow and working with Facebook's Graph API. In Spring Boot they are the `spring.social.facebook.appId` and `spring.social.facebook.appSecret` respecytively.
